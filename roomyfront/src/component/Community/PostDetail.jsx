...
import { fetchPostById, deletePost } from "../../api/CommunityApi";
import { toggleLikePost, getLikeCount } from "../../api/LikeApi";
...
const PostDetail = () => {
  ...
  const userId = "3"; // 현재 로그인한 사용자 ID (테스트 값, 실제 구현에서는 동적으로 할당)
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      const handleAddComment = async () => {
        if (newComment.trim()) {
          try {
            const newCommentData = await addComment(communityId ,userId, newComment); // userId를 1로 예시 지정
      ...
      const handleAddReply = async (commentId) => {
        ...
        if (reply[commentId]?.trim()) {
          try {
            ...
            console.log("📌 대댓글 API 요청 데이터:", { commentId, userId, content: reply[commentId] });
            
            const newReplyData = await addReply(commentId, userId, reply[commentId]);
  ...
  const handleAddComment = async () => {
    if (newComment.trim()) {
      try {
        ...
        const newCommentData = await addComment(communityId, userId, newComment);
    ...
    <div className="post-detail">
      ...
      <div className="post-header">
        <div className="profile">
          <img src={post.profileImage || sampleImage} alt="프로필" className="profile-image" />
        ...
        <div className="menu-container">
          ...
          {menuOpen && (
            <ul className="menu-dropdown">
              ...
              <li>
                <button className="report-button" onClick={() => setIsReportOpen(true)}>신고</button>
              </li>
            </ul>
          )}
        </div>
            ...
            <li key={comment.id} className="comment-item">
              <div className="comment-header">
                ...
                <div className="comment-info">
                  ...
                  {comment.createdAt ? getRelativeTime(comment.createdAt) : "시간 없음"}
                  </span>
                </div>
              </div>
